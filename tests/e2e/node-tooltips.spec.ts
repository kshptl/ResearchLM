...
test("node footer action tooltips render with non-transparent shadcn popover styling", async ({ page }) => {
  await page.goto("/")
  ...
  const tooltip = page.locator(".bg-foreground").filter({ hasText: "Follow up" }).first()
